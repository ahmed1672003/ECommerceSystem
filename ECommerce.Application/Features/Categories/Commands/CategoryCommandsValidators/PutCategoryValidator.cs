...
public class PutCategoryValidator : AbstractValidator<PutCategoryCommand>
{
    ...
    private void ApplyValidatorRules()
    {
        ...
        .NotEmpty().WithMessage(
            c => $"{nameof(c.CategoryDTO.Name)} can not be not empty")
        .NotNull().WithMessage(
            c => $"{nameof(c.CategoryDTO.Name)} can not be not null")
        .MaximumLength(100).WithMessage(
            c => $"{nameof(c.CategoryDTO.Name)} his length can not be bigger than {100}")
        .MinimumLength(1).WithMessage(
            c => $"{nameof(c.CategoryDTO.Name)} his length can not be less than {1}");
    }
    private void ApplyCustomValidatorRules()
    {
        ...
        await _context.Categories.IsExist(
            c => c.Id.Equals(command.Id) && c.Id.Equals(command.CategoryDTO.Id)))
        .WithMessage(
        ...
        RuleFor(c => c)
           ...
           await _context.Categories.CanUpdatedAsync(command.CategoryDTO.Name, command.Id))
            .WithMessage(
            c => $"{nameof(c.CategoryDTO.Name)} is Exist !");
    }
}
